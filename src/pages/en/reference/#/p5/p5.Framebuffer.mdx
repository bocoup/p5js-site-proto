...
pixels:
  ...
  path: ./src/pages/en/reference/#/p5.Framebuffer/
resize:
  description: |
...
color:
  description: >
    <p>A texture with the color information of the framebuffer. Pass this (or
    the
    ...
    <p>Since Framebuffers are controlled by WebGL, their y coordinates are
    stored
...
depth:
  description: >
    <p>A texture with the depth information of the framebuffer. If the
    framebuffer

    was created with <code>{ depth: false }</code> in its settings, then this
    property will

    be undefined. Pass this to <a href="#/p5/texture">texture()</a> to draw it
    to
